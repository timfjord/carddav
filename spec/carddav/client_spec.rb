...
describe Carddav::Client do
  ...
  describe 'contructor' do
    it 'should set variables' do
      client = Carddav::Client.new host, 'username', 'password'
  ...
  describe 'url setters' do
    let(:client) { Carddav::Client.new host, 'username', 'password' }

    it 'should set url and append to host if needed' do
      [:current_user_principal_url, :addressbook_home_set_url, :addressbook_url].each do |method|
        client.send "#{method}=",'/path'
        expect(client.send(method).to_s).to eql 'http://test.com/path'

        client.send "#{method}=", 'http://new.test.com/new_path'
